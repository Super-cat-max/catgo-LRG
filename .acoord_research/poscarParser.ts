...
import { getDefaultAtomRadius, parseElement } from '../../utils/elementData.js'
 ...
 */
export class POSCARParser extends StructureParser {
  ...
  serialize(structure: Structure): string {
    ...
    for (const atom of orderedAtoms) {
      ...
      if (hasSelectiveDynamics) {
        ...
        const flags = sd.map((f) => f ? `T` : `F`).join(`  `)
        row += `  ${flags}`
      }
      lines.push(row)
    }
  ...
  private parseHeader(lines: string[]): ParsedHeader {
    ...
    if (this.tokensAreAllIntegers(firstDataTokens)) {
      counts = firstDataTokens.map((token) => parseInt(token, 10)).filter((v) =>
      ...
      counts = countTokens.map((token) => parseInt(token, 10)).filter((v) =>
        Number.isFinite(v) && v >= 0
      )
    }
    ...
    const looksLikeCoordinate = modeTokens.length >= 3 &&
  ...
  private parseScalingFactors(line: string): number[] {
    ...
    const values = tokens.slice(0, 3).map((token) => this.parseNumber(token)).filter((
      value,
  ...
  private parseVector(line: string): number[] {
    const values = this.tokenizeDataLine(line).slice(0, 3).map((token) =>
