...
export class XDATCARParser extends StructureParser {
      ...
      for (const element of elementOrder) {
        ...
        for (const atomIndex of atomIndices) {
          ...
          if (atom.element !== element) {
            throw new Error(
              `XDATCAR export requires consistent atom ordering across frames`,
            )
          }
          let fx = atom.x
          let fy = atom.y
  ...
  private tryParseHeader(lines: string[], startIndex: number): XdatcarHeader | null {
    ...
    if (!isCoordinateLine) {
      ...
      coordinateMode = token.startsWith(`c`) || token.startsWith(`k`)
        ? `cartesian`
        : `direct`
      modeIndex++
    }
  ...
  private buildFrame(
    ...
    for (let i = 0; i < atomCount; i++) {
      ...
      structure.addAtom(
        new Atom(expandedElements[i], x, y, z, undefined, {
