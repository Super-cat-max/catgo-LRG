...
export class Structure {
  ...
  getBonds(): Array<
    { atomId1: string; atomId2: string; distance: number; radius: number; color?: string }
  > {
    return this.bonds
      .map((bond) => {
  ...
  calculateBonds(schemeId?: BondSchemeId): void {
    ...
    const atomData = new Map<
      string,
      { symbol: string; radius: number; atomicNumber: number }
    >()
    for (const atom of this.atoms) {
  ...
  private calculateNonPeriodicBonds(
    ...
    for (const atom1 of this.atoms) {
      ...
      for (
        const atom2 of this.getNeighboringAtoms(atom1, grid, cellSize, maxBondLength)
      ) {
  ...
  static fromJSON(data: ReturnType<Structure[`toJSON`]>): Structure {
    ...
    const legacyManualBonds = (data as Record<string, unknown>).manualBonds as
      | Array<[string, string]>
      | undefined
