...
function getSurfaceShininess(): number {
  const value = Number(getDisplayStore().shininess)
...
function createCamera(
  ...
  if (mode === `orthographic`) {
    ...
    camera.zoom = getDisplayStore().viewZoom || 1
...
export class StructureRenderer {
  constructor(private readonly handlers: RendererHandlers) {}

  init(canvas: HTMLCanvasElement): void {
    rendererState.setError = this.handlers.setError
    rendererState.setStatus = this.handlers.setStatus

    const container = document.getElementById(`container`)
    if (!container) {
      this.handlers.setError(`Container element not found.`)
      return
    }
    ...
    this.handlers.setStatus(
      `Canvas size: ` + Math.round(width) + `x` + Math.round(height),
    )

    const scene = new THREE.Scene()
    scene.background = new THREE.Color(getDisplayStore().backgroundColor || `#0d1015`)
    rendererState.scene = scene

    rendererState.projectionMode = getDisplayStore().projectionMode || `perspective`
    ...
    try {
      ...
      this.handlers.setError(
        `WebGL renderer failed to initialize. Check GPU/WebGL support.`,
      )
      return
    }
    ...
    if (!gl) {
      this.handlers.setError(
        `WebGL context unavailable. Your system or VS Code may have WebGL disabled.`,
      )
      return
    }

    rendererState.ambientLight = new THREE.AmbientLight(
      resolveLightColor(getLightingStore().ambientColor, `#ffffff`),
      // Three.js r183 BRDF_Lambert divides diffuse intensity by PI in the shader.
      // Multiply intensity by PI to compensate and match old r128 brightness.
      (getLightingStore().ambientIntensity ?? 0.5) * Math.PI,
    )
    rendererState.scene.add(rendererState.ambientLight)

    rendererState.keyLight = new THREE.DirectionalLight(
      resolveLightColor(getLightingStore().keyLight?.color, `#CCCCCC`),
      (getLightingStore().keyLight?.intensity ?? 0.7) * Math.PI,
    )
    rendererState.scene.add(rendererState.keyLight)

    rendererState.fillLight = new THREE.DirectionalLight(
      resolveLightColor(getLightingStore().fillLight?.color, `#ffffff`),
      (getLightingStore().fillLight?.intensity ?? 0) * Math.PI,
    )
    rendererState.scene.add(rendererState.fillLight)

    rendererState.rimLight = new THREE.DirectionalLight(
      resolveLightColor(getLightingStore().rimLight?.color, `#ffffff`),
      (getLightingStore().rimLight?.intensity ?? 0) * Math.PI,
    )
    rendererState.scene.add(rendererState.rimLight)
    ...
    rendererState.statusInterval = setInterval(() => {
      ...
      this.handlers.setStatus(`Draw calls: ` + calls)
    }, 1000)
  }
  ...
  getAtomMeshes(): Map<string, THREE.Mesh> {
    return getAtomMeshes()
  }

  getBondMeshes(): THREE.Mesh[] {
    return getBondMeshes()
  }
  ...
  exportHighResolutionImage(
    ...
    return exportHighResolutionImage(options)
  }
...
function updateLightsForCamera(): void {
  if (
    !rendererState.camera || !rendererState.keyLight ||
  ...
  _keyOffset.set(
    getLightingStore().keyLight?.x ?? 0,
    getLightingStore().keyLight?.y ?? 0,
    getLightingStore().keyLight?.z ?? 10,
  )
  _fillOffset.set(
    getLightingStore().fillLight?.x ?? -10,
    getLightingStore().fillLight?.y ?? -5,
    getLightingStore().fillLight?.z ?? 5,
  )
  _rimOffset.set(
    getLightingStore().rimLight?.x ?? 0,
    getLightingStore().rimLight?.y ?? 5,
    getLightingStore().rimLight?.z ?? -10,
  )
...
function onResize(): void {
  ...
  if (rendererState.camera instanceof THREE.OrthographicCamera) {
    ...
    rendererState.camera.zoom = getDisplayStore().viewZoom || 1
...
function getAutoScales(atoms: Atom[]): { scale: number; sizeScale: number } {
  ...
  const scale = Math.min(
    Math.max(CAMERA_TARGET_DIMENSION / maxDim, CAMERA_SCALE_MIN),
    CAMERA_SCALE_MAX,
  )
  const sizeScale = Math.min(
    Math.max(10 / Math.sqrt(maxDim), CAMERA_SIZE_SCALE_MIN),
    CAMERA_SIZE_SCALE_MAX,
  )
...
function createUnitCellEdgeMesh(
  start: THREE.Vector3,
  end: THREE.Vector3,
  radius: number,
  color: string,
...
function buildUnitCellGroup(edges: UnitCellEdge[], scale: number): THREE.Group | null {
  ...
  const color = getDisplayStore().unitCellColor || `#FF6600`
  const thickness = Number.isFinite(getDisplayStore().unitCellThickness)
    ? Math.max(0.5, Math.min(6, getDisplayStore().unitCellThickness))
    : 1
  const style = getDisplayStore().unitCellLineStyle === `dashed` ? `dashed` : `solid`
...
function renderStructure(
  data: Structure,
  uiHooks?: Partial<UiHooks>,
  options?: { fitCamera?: boolean },
): void {
  let scale = getDisplayStore().manualScale
  let sizeScale = 1
  if (getDisplayStore().autoScaleEnabled) {
  ...
  if (renderAtoms) {
    ...
    const selectableAtoms = renderAtoms.filter((atom) => {
      if (
        !Number.isFinite(atom.position[0]) || !Number.isFinite(atom.position[1]) ||
        !Number.isFinite(atom.position[2])
      ) return false
  ...
  if (renderBonds) {
    ...
    for (const bond of renderBonds) {
      ...
      const highlightBond = isSelectedBond ||
        !!(bond.atomId1 && bond.atomId2 && selectedSet.has(bond.atomId1) &&
      ...
      const bondRadius = Math.max(bond.radius * sizeScale, 0.03) *
        (highlightBond ? 1.35 : 1)
      ...
      bondHalves.push({
        ...
        ownAtomId: bond.atomId1,
        otherAtomId: bond.periodicStub ? bond.atomId1 : bond.atomId2,
        isFirstHalf: true,
      ...
      bondHalves.push({
        ...
        to: end,
        halfLen: length / 2,
        color: bond.color2 || bond.color,
        bondRadius,
        emissive,
        bondKey: bond.key,
        ownAtomId: bond.periodicStub ? bond.atomId1 : bond.atomId2,
        otherAtomId: bond.atomId1,
        isFirstHalf: false,
        fixedEnd: bond.periodicStub ? end.clone() : null,
      })
    }
    ...
    for (const [, halves] of byBondRadius) {
      ...
      im.instanceColor = new THREE.InstancedBufferAttribute(
        new Float32Array(halves.length * 3),
        3,
      )
  ...
  const fixedAtoms = renderAtoms
    ? renderAtoms.filter((atom) => atom.fixed && atom.selectable !== false)
    : []
  ...
  if (uiHooks) {
    ...
    if (uiHooks.updateAtomList) {
      uiHooks.updateAtomList(
        data.atoms,
        data.selectedAtomIds || [],
        data.selectedAtomId || null,
      )
    }
  }
...
function fitCamera(): void {
  ...
  if (rendererState.camera instanceof THREE.OrthographicCamera) {
    ...
    rendererState.orthoSize = targetSize / (getDisplayStore().viewZoom || 1)
    ...
    const cameraDistance = Math.abs(maxDim / 2 / Math.tan(fov / 2)) * 0.6 /
      getDisplayStore().viewZoom
...
function getAtomMeshes(): Map<string, THREE.Mesh> {
  return rendererState.atomMeshes
}
function getBondMeshes(): THREE.Mesh[] {
  return rendererState.bondMeshes
}
function getDragPlane(): THREE.Plane {
...
function updateLighting(): void {
  if (
    !rendererState.ambientLight || !rendererState.keyLight ||
    !rendererState.fillLight || !rendererState.rimLight || !rendererState.camera
  ) {
    return
  }
  const enabled = getLightingStore().lightingEnabled !== false
  rendererState.ambientLight.intensity = enabled
    ? (getLightingStore().ambientIntensity ?? 0.5) * Math.PI
    : 0
  rendererState.keyLight.intensity = enabled
    ? (getLightingStore().keyLight?.intensity ?? 0.7) * Math.PI
    : 0
  rendererState.fillLight.intensity = enabled
    ? (getLightingStore().fillLight?.intensity ?? 0) * Math.PI
    : 0
  rendererState.rimLight.intensity = enabled
    ? (getLightingStore().rimLight?.intensity ?? 0) * Math.PI
    : 0
  rendererState.ambientLight.color.set(
    resolveLightColor(getLightingStore().ambientColor, `#ffffff`),
  )
  rendererState.keyLight.color.set(
    resolveLightColor(getLightingStore().keyLight?.color, `#CCCCCC`),
  )
  rendererState.fillLight.color.set(
    resolveLightColor(getLightingStore().fillLight?.color, `#ffffff`),
  )
  rendererState.rimLight.color.set(
    resolveLightColor(getLightingStore().rimLight?.color, `#ffffff`),
  )
...
function updateDisplaySettings(): void {
  ...
  if (rendererState.unitCellGroup && getDisplayStore().unitCellColor) {
    const nextColor = new THREE.Color(getDisplayStore().unitCellColor)
    rendererState.unitCellGroup.traverse((node: THREE.Object3D) => {
      const mesh = node as THREE.Mesh
      if (!mesh.material) return
      const materials = Array.isArray(mesh.material) ? mesh.material : [mesh.material]
      for (const material of materials) {
...
function exportHighResolutionImage(
  ...
  const requestedScale = options && Number.isFinite(Number(options.scale))
  ...
  const perspectiveAspect = camera instanceof THREE.PerspectiveCamera
    ? camera.aspect
    : null
  ...
  const maxTextureSize =
    (renderer.capabilities as { maxTextureSize?: number })?.maxTextureSize || 8192
