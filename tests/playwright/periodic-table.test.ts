...
test.describe(`Periodic Table`, () => {
  ...
  test.skip(`in default state`, async ({ page }) => {
    ...
    expect(element_tiles).toHaveLength(
      ELEMENT_COUNT + n_lanthanide_actinide_placeholders,
    )
  ...
  test.describe(`in heatmap mode`, () => {
    test(`displays elemental heat values`, async ({ page }) => {
      ...
      // select all heatmaps in sequence making sure none of them crash
      ...
      // Verify that element tiles display numeric values when heatmap is selected
      // Check random tiles have a third line (the heatmap value)
      const tiles = await page.$$(`.element-tile`)
      const sample_tiles = random_sample(tiles, 5)

      for (const tile of sample_tiles) {
        // Element tiles should have a value displayed (number/symbol/value format)
        const text = await tile.textContent()
        // Should have at least number and symbol, and potentially a value
        expect(text).toBeTruthy()
        // The tile text should contain numbers (atomic number at minimum)
