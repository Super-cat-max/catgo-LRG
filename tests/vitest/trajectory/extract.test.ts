...
describe(`Full Data Extractor`, () => {
  ...
  it(`should detect constant lattice parameters`, () => {
    ...
    // The extractor no longer emits `_constant_{a,b,c,alpha,beta,gamma}` keys
    // (they were unused downstream — variance detection now happens inline).
    // We only verify that the constant lattice values themselves match across
    // frames.
    expect(frame1_data.a).toBe(frame2_data.a)
    expect(frame1_data.b).toBe(frame2_data.b)
    expect(frame1_data.c).toBe(frame2_data.c)
...
describe(`HDF5 Trajectory Data Extraction`, () => {
  ...
  it(`should handle all frames and lattice consistency`, async () => {
    ...
    // Check lattice consistency by comparing raw values frame-to-frame —
    // the extractor no longer emits `_constant_*` markers (see commentary
    // above structural_data_extractor in src/lib/trajectory/extract.ts).
    const ref_a = all_frame_data[0].a
    const ref_b = all_frame_data[0].b
    const ref_c = all_frame_data[0].c
    const ref_alpha = all_frame_data[0].alpha
    const ref_beta = all_frame_data[0].beta
    const ref_gamma = all_frame_data[0].gamma
    const all_same = all_frame_data.every((data: Record<string, unknown>) =>
      data.a === ref_a && data.b === ref_b && data.c === ref_c &&
      data.alpha === ref_alpha && data.beta === ref_beta && data.gamma === ref_gamma
    )
    expect(typeof all_same).toBe(`boolean`)
  })
})
