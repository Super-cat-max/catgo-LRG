...
describe(`Structure`, () => {
  ...
  test.each(
    ...
    // Find the .export-item whose <span> label matches the format
    const export_items = Array.from(document.querySelectorAll(`.export-item`))
    const format_item = export_items.find(
      (item) => item.querySelector(`span`)?.textContent?.trim() === format,
    )
    expect(format_item, `export-item for ${format}`).toBeTruthy()

    if (action === `Download`) {
      globalThis.URL.createObjectURL = vi.fn()
      const spy = vi.spyOn(document.body, `appendChild`)
      const download_btn = format_item!.querySelector<HTMLButtonElement>(
      ...
      const copy_btn = format_item!.querySelector<HTMLButtonElement>(
        `button[title="Copy"]`,
      )
...
describe(`Structure component nested JSON handling`, () => {
  ...
  test.each([
    ...
    if (!structure) {
      ...
      expect(document.body.textContent).toContain(`No atoms in structure`)
    }
  })
...
test.each([
  ...
  async (initial_projection, target_projection) => {
    ...
    // In test environment, scene_props may not propagate to inputs immediately
    // Just verify the inputs exist and have valid numeric values
    expect(radius_input).toBeTruthy()
    expect(parseFloat(radius_input?.value || `0`)).toBeGreaterThan(0)
    expect(auto_rotate_input).toBeTruthy()
