...
// ─── Phase 1: jimage propagation through TS bond pipeline ───
//
...
//
// These tests cover the JS-fallback path and the WasmBond → BondPair
...
describe(`Phase 1 jimage propagation`, () => {
  ...
  test(`wasm_bonds_to_pairs preserves jimage from Rust image field`, async () => {
    const { wasm_bonds_to_pairs } = await import('$lib/structure/workers/bond-worker-api')
    const structure = get_test_structure([
      { xyz: [0, 0, 0], element: `Na` },
      { xyz: [3, 0, 0], element: `Cl` },
    ])
    // Simulate Rust → JSON wire format: include non-zero image to make sure
    // the controller-side ingest does NOT silently drop it like it used to.
    const wasm_bonds = [
      { site_idx_1: 0, site_idx_2: 1, bond_length: 3.0, strength: 1, image: [0, 0, 0] as [number, number, number] },
      { site_idx_1: 0, site_idx_2: 1, bond_length: 5.83, strength: 1, image: [1, 0, 0] as [number, number, number] },
      { site_idx_1: 0, site_idx_2: 1, bond_length: 5.83, strength: 1, image: [-1, 0, 0] as [number, number, number] },
    ]
    const pairs = wasm_bonds_to_pairs(wasm_bonds, structure)
    expect(pairs).toHaveLength(3)
    expect(pairs[0].jimage).toEqual([0, 0, 0])
    expect(pairs[1].jimage).toEqual([1, 0, 0])
    expect(pairs[2].jimage).toEqual([-1, 0, 0])
  })

  test(`detect_hydrogen_bonds emits jimage = [0, 0, 0] on every H-bond`, () => {
    // Simple D-H···A geometry: O-H · · · O at ~120° angle, H-A ~1.9 Å.
    const structure = get_test_structure([
      { xyz: [0, 0, 0], element: `O` }, // donor
      { xyz: [0.95, 0, 0], element: `H` }, // hydrogen (covalently bonded to donor)
      { xyz: [2.85, 0, 0], element: `O` }, // acceptor
    ])
    // Pre-computed covalent bond list for the donor's O-H pair.
    const covalent: BondPair[] = [{
      pos_1: structure.sites[0].xyz,
      pos_2: structure.sites[1].xyz,
      site_idx_1: 0,
      site_idx_2: 1,
      bond_length: 0.95,
      strength: 1,
      transform_matrix: bonding.compute_bond_transform(
        structure.sites[0].xyz,
        structure.sites[1].xyz,
      ),
      jimage: [0, 0, 0],
    }]
    const hbonds = bonding.detect_hydrogen_bonds(structure, covalent, {
      max_ha_distance: 2.5,
      max_da_distance: 3.5,
      min_angle: 120,
    })
    for (const hb of hbonds) {
      expect(hb.jimage).toEqual([0, 0, 0])
      expect(hb.bond_type).toBe(`hydrogen`)
    }
  })
})
