...
describe(`handle_legend_double_click`, () => {
  ...
  test(`handles new series added after isolation - restores when isolated`, () => {
    // D is false (out-of-group), so B appears isolated → restores prev_visibility
    const series: DataSeries[] = [
      { x: [1], y: [2], label: `A`, visible: false },
      { x: [3], y: [4], label: `B`, visible: true },
      { x: [5], y: [6], label: `C`, visible: false },
      { x: [7], y: [8], label: `D`, visible: false },
    ]
    const result = handle_legend_double_click(series, 1, [true, true, true])
    // First 3 restored from prev_visibility, D keeps current (idx >= prev.length)
    expect(result.series.map((srs) => srs.visible)).toEqual([true, true, true, false])
    expect(result.previous_visibility).toBe(null)
  })

  test(`handles new series added after isolation - isolates when not isolated`, () => {
    // D is true (visible out-of-group), so B is NOT isolated → isolates B
    const series: DataSeries[] = [
      { x: [1], y: [2], label: `A`, visible: false },
      { x: [3], y: [4], label: `B`, visible: true },
      { x: [5], y: [6], label: `C`, visible: false },
      { x: [7], y: [8], label: `D`, visible: true },
    ]
    const result = handle_legend_double_click(series, 1, [true, true, true])
    expect(result.series.map((srs) => srs.visible)).toEqual([false, true, false, false])
  })
  ...
  test.each([
    ...
    // Invalid index: no series matches in_group, so all get set to false (isolation attempt)
    expect(result.series.map((srs) => srs.visible)).toEqual([false, false])
    expect(result.previous_visibility).toEqual([true, true])
  })
})
