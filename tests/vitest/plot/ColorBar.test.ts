...
describe(`ColorBar Vertical`, () => {
  test(`renders correctly with default vertical props`, () => {
    ...
    const cbar_div = doc_query(`.colorbar > div.bar`)
    // happy-dom doesn't resolve CSS variables in computed styles. The
    // component sets `--cbar-width` and `--cbar-height` inline and the
    // `--cbar-thickness` default (14px) appears as the var() fallback for
    // the vertical-orientation width.
    const inline_style = cbar_div.getAttribute(`style`) ?? ``
    expect(inline_style).toMatch(/--cbar-width:\s*var\(--cbar-thickness,\s*14px\)/)
