...
describe(`ScatterPoint`, () => {
  ...
  describe(`Tween Behavior`, () => {
    ...
    test(`applies custom point_tween options (easing)`, async () => {
      ...
      // Primary check is that mounting with custom easing doesn't error.
      // In happy-dom, SVG tweens may not fully resolve, so we just verify
      // the transform attribute exists and is a valid translate().
      expect(g_element.getAttribute(`transform`)).toMatch(
        /^translate\([\d. -]+ [\d. -]+\)$/,
      )
    })
  })
