...
describe(`Theme System`, () => {
  ...
  describe(`Theme preference storage`, () => {
    test.each([...Object.keys(COLOR_THEMES), `auto`])(
      ...
      (theme) => {
        ...
        expect(localStorage.getItem(`catgo-theme`)).toBe(theme)
      },
    )
    ...
    test.each([...Object.keys(COLOR_THEMES), `auto`])(
      ...
      (theme) => {
        localStorage.setItem(`catgo-theme`, theme)
  ...
  describe(`DOM theme application`, () => {
    test.each(Object.keys(COLOR_THEMES))(
      ...
      (theme) => {
        ...
        const expected = globalThis.CATGO_THEMES[theme as ThemeName] as {
          surface_bg: string
          text_color: string
        }
    ...
    test(`apply_theme_to_dom handles missing global data gracefully`, () => {
      ...
      globalThis.CATGO_THEMES = undefined
      // @ts-expect-error - setting bad types for testing
      globalThis.CATGO_CSS_MAP = undefined
  ...
  describe(`Theme data integrity`, () => {
    test(`all theme keys have complete variant coverage`, () => {
      if (!globalThis.CATGO_THEMES) return
      const theme_names = Object.keys(globalThis.CATGO_THEMES)
      const first_theme = globalThis.CATGO_THEMES[theme_names[0] as ThemeName]
      ...
      for (const theme_name of theme_names) {
        const theme_keys = Object.keys(
          globalThis.CATGO_THEMES[theme_name as ThemeName],
        )
