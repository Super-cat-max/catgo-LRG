...
describe(`Composition component`, () => {
  ...
  test(`export options are available in context menu`, async () => {
    ...
    // Context menu may render via portal causing duplicate DOM nodes;
    // verify at least the expected 4 options exist
    expect(export_options.length).toBeGreaterThanOrEqual(4)

    const option_texts = [...new Set(export_options.map((opt) => opt.textContent?.trim()))]
    expect(option_texts).toContain(`Copy Formula`)
