...
test(`parse_formula_with_oxidation handles various oxidation state formats`, () => {
  // Caret syntax: only `digit+sign` (e.g. 2+, 2-) and bare sign (±1) are unambiguous
  // `^+2` is parsed as bare `+` (oxidation +1) with count 2
  // Bracket syntax supports all formats: [+2], [2+], [-2], [2-]
  const test_cases: Array<[string, number]> = [
    [`Fe^2+O`, 2],
    [`Fe^2-O`, -2],
    [`Fe[+2]O`, 2],
  ...
  // Caret with sign+number: sign is oxidation, number is count
  const fe_plus2 = parse_formula_with_oxidation(`Fe^+2O`)
  expect(fe_plus2[0].oxidation_state).toBe(1)
  expect(fe_plus2[0].amount).toBe(2)

  const fe_minus2 = parse_formula_with_oxidation(`Fe^-2O`)
  expect(fe_minus2[0].oxidation_state).toBe(-1)
  expect(fe_minus2[0].amount).toBe(2)
})
...
test(`Formula formats amounts with custom format string`, () => {
  // Parser uses integer regex (\d*), so H2.5O parses as H(2) + O(1)
  // The .2f format applies to the parsed integer amount
  const { container } = render(Formula, {
...
test.each([
  ...
  // SO4^2- and NH4^+: charge comes after count, parser doesn't attach it to an element
  { formula: `SO4^2-`, expected_elements: [`S`, `O`], expected_superscripts: 0 },
  { formula: `NH4^+`, expected_elements: [`N`, `H`], expected_superscripts: 0 },
])(
