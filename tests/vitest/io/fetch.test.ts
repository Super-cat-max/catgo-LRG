...
describe(`download`, () => {
  ...
  test(`downloads string data using default browser download`, async () => {
    download(`test data`, `test.txt`, `text/plain`)

    // download() now tries showSaveFilePicker first (async), falls back to <a> link
    // Flush microtask queue so the async fallback path runs
  ...
  test(`downloads blob data using default browser download`, async () => {
