...
// Derive an orbits array (input-cell index → representative atom index) by
// grouping atoms with the same Wyckoff letter + atomic number. The current
// wyckoff_positions_from_moyo implementation walks orbits to group sites,
// and these tests build minimal MoyoDataset mocks that don't natively
// include the field — derive it here so the existing mocks keep working.
function derive_orbits(
  numbers: number[],
  wyckoffs: ReadonlyArray<string | null>,
): number[] {
  const group_to_rep = new Map<string, number>()
  return numbers.map((num, idx) => {
    const wf = (wyckoffs[idx] ?? ``).toString()
    const letter = wf.match(/[a-z]+$/)?.[0] ?? wf
    const key = `${letter}|${num}`
    if (!group_to_rep.has(key)) group_to_rep.set(key, idx)
    return group_to_rep.get(key) as number
  })
}

describe(`wyckoff_positions_from_moyo`, () => {
  // Build a minimal MoyoDataset for the unit tests. wyckoff_positions_from_moyo
  // requires an `orbits` array (input-cell-index → representative atom index)
  // to group sites; derive it here by grouping atoms with the same Wyckoff
  // letter + atomic number so callers only have to specify the user-facing
  // fields.
  const mock_data = (
    positions: number[][],
    numbers: number[],
    ...
    const group_to_rep = new Map<string, number>()
    const orbits = positions.map((_, idx) => {
      const wf = (wyckoffs[idx] ?? ``).toString()
      const letter = wf.match(/[a-z]+$/)?.[0] ?? wf
      const key = `${letter}|${numbers[idx]}`
      if (!group_to_rep.has(key)) group_to_rep.set(key, idx)
      return group_to_rep.get(key) as number
    })
    return { std_cell: { positions, numbers }, wyckoffs, orbits } as unknown as MoyoDataset
  }
...
describe(`stable atom ordering`, () => {
  test(`wyckoff positions maintain stable ordering across multiple calls`, () => {
    // orbits maps input-cell index → representative atom index. The H atoms
    // (numbers 1) at indices 0/2/3 share wyckoff `2a` → same rep; the O atom
    // at index 1 is in its own orbit.
    const mock_data = {
      std_cell: {
...
describe(`site coverage verification`, () => {
  test(`all sites are covered by wyckoff positions`, () => {
    ...
    for (const test_case of test_cases) {
      const mock_data = {
        ...
        orbits: derive_orbits(test_case.numbers, test_case.wyckoffs),
  ...
  test(`handles edge cases in Wyckoff position parsing`, () => {
    ...
    edge_cases.forEach(({ mock_data, expected }) => {
      const md = mock_data as { std_cell: { numbers: number[] }; wyckoffs: (string | null)[] }
      const with_orbits = {
        ...mock_data,
        orbits: derive_orbits(md.std_cell.numbers, md.wyckoffs),
      } as unknown as MoyoDataset
      const result = wyckoff_positions_from_moyo(with_orbits)
      expect(result).toEqual(expected)
    })
  })

  test(`performance with very large structures`, () => {
    const numbers = Array.from({ length: 1000 }, () => Math.floor(Math.random() * 10) + 1)
    const wyckoffs = Array.from({ length: 1000 }, (_, idx) => `${(idx % 10) + 1}a`)
    const large_structure = {
      ...
      wyckoffs,
      orbits: derive_orbits(numbers, wyckoffs),
    } as unknown as MoyoDataset
