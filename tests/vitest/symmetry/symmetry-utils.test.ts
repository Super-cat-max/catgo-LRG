...
describe(`wyckoff_positions_from_moyo`, () => {
  // wyckoff_positions_from_moyo groups sites via `orbits` (input-cell index →
  // representative atom index). Derive it from the supplied
  // numbers + wyckoffs so tests only have to spell out the user-facing fields.
  const make_sym_data = (
    positions: number[][],
    numbers: number[],
    ...
    const orbits = numbers.map((num, idx) => {
  ...
  test(`should handle original indices and sorting`, () => {
    // site_indices reflect input-cell positions (which start at 0); the
    // optional `orig_indices` field on the dataset isn't used by
    // wyckoff_positions_from_moyo, so this single-site row remains [0].
    const with_indices = wyckoff_positions_from_moyo(
      make_sym_data([[0, 0, 0]], [6], [`1a`], [42]),
    )
    expect(with_indices[0].site_indices).toEqual([0])
