<script lang="ts">
  ...
  // Merge defaults reactively so parent prop updates flow through
  const _styles = $derived({
    show_points: DEFAULTS.scatter.show_points,
    show_lines: DEFAULTS.scatter.show_lines,
    ...styles,
    point: { ...DEFAULTS.scatter.point, ...styles.point },
    line: { ...DEFAULTS.scatter.line, ...styles.line },
  })
  const _x = $derived({
    format: `` as string, scale_type: `linear` as const,
    label_shift: { x: 0, y: -40 }, tick_label_shift: { x: 0, y: 20 },
    range: [null, null] as [number | null, number | null],
    ...x_axis,
  })
  const _y = $derived({
    format: `` as string, scale_type: `linear` as const,
    label_shift: { x: 0, y: 0 }, tick_label_shift: { x: -8, y: 0 },
    range: [null, null] as [number | null, number | null],
    ...y_axis,
  })
  const _y2 = $derived({
    format: `` as string, scale_type: `linear` as const, ticks: 5,
    label_shift: { x: 0, y: 0 }, tick_label_shift: { x: 8, y: 0 },
    range: [null, null] as [number | null, number | null],
    ...y2_axis,
  })
  const _display = $derived({ ...DEFAULTS.scatter.display, ...display })
  const _controls = $derived({ show: true, open: false, ...controls })
  ...
  let pad = $state({ ...default_padding, ...untrack(() => padding) })
  ...
  let auto_x_range = $derived(
    get_nice_data_range(
      all_points,
      ({ x }) => x,
      (_x.range ?? [null, null]) as [number | null, number | null],
      _x.scale_type!,
      range_padding,
      _x.format?.startsWith(`%`) || false,
    ),
  )
  ...
  let auto_y_range = $derived(
    get_nice_data_range(
      y1_points,
      ({ y }) => y,
      (_y.range ?? [null, null]) as [number | null, number | null],
      _y.scale_type!,
      range_padding,
      false,
    ),
  ...
  let auto_y2_range = $derived(
    get_nice_data_range(
      y2_points,
      ({ y }) => y,
      (_y2.range ?? [null, null]) as [number | null, number | null],
      _y2.scale_type!,
      range_padding,
      false,
    ),
  ...
  $effect(() => {
    const new_x = [
      _x.range?.[0] ?? auto_x_range[0],
      _x.range?.[1] ?? auto_x_range[1],
    ] as [number, number]
    const new_y = [
      _y.range?.[0] ?? auto_y_range[0],
      _y.range?.[1] ?? auto_y_range[1],
    ] as [number, number]
    const new_y2 = [
      _y2.range?.[0] ?? auto_y2_range[0],
      _y2.range?.[1] ?? auto_y2_range[1],
    ] as [number, number]
    ...
    series_with_ids
      .map((data_series: DataSeries, series_idx: number) => {
        ...
        if (!data_series) { // Return empty data consistent with DataSeries structure
          return { x: [], y: [], visible: true, filtered_data: [], _id: series_idx } as any
        }
  ...
  let plot_points_for_placement = $derived.by(() => {
    ...
    for (const series_data of filtered_series) {
      ...
      for (const point of series_data.filtered_data) {
        const point_x_coord = _x.format?.startsWith(`%`)
  ...
  let axis_ticks = $derived.by(() => {
    ...
    const x_scale_for_ticks = _x.format?.startsWith(`%`)
  ...
  function update_tooltip_point(x_rel: number, y_rel: number, evt?: MouseEvent) {
    ...
    for (const series_data of filtered_series) {
      ...
      for (const point of series_data.filtered_data) {
        ...
        const point_cx = _x.format?.startsWith(`%`)
          ? x_scale_fn(new Date(point.x))
          : x_scale_fn(point.x)
  ...
  let actual_label_config = $derived({
    ...
    link_distance_range: [5, 20] as [number | null, number | null], // Default min and max distance (replacing max_link_distance)
  ...
  $effect(() => {
    ...
    label_positions = compute_label_positions(
      filtered_series,
      actual_label_config,
      { x_scale_fn: x_scale_fn as ScaleContinuousNumeric<number, number>, y_scale_fn, y2_scale_fn, x_axis },
      { width, height, pad },
    )
  })
  ...
  function get_screen_coords(point: Point, series?: DataSeries): [number, number] {
    ...
    const screen_x = _x.format?.startsWith(`%`)
      ? x_scale_fn(new Date(point.x))
      : x_scale_fn(point.x)
    ...
    const min_domain_y = use_y2
      ? _y2.scale_type === `log` ? y_scale.domain()[0] : -Infinity
      : _y.scale_type === `log`
      ? y_scale.domain()[0]
      : -Infinity
    const safe_y_val = use_y2
      ? _y2.scale_type === `log` ? Math.max(y_val, min_domain_y) : y_val
      : _y.scale_type === `log`
  ...
  function construct_handler_props(
    ...
    return {
      ...
      series_idx,
      x_formatted: format_value(x, _x.format || `.3~s`),
      y_formatted: format_value(
        y,
        (hovered_series.y_axis === `y2` ? _y2.format : _y.format) || `.3~s`,
      ),
  ...
  let using_controls = $derived(_controls.show)
  ...
  {#if width && height}
    ...
    >
      ...
      <g class="x-axis">
        {#if width > 0 && height > 0}
          {#each x_tick_values as tick (tick)}
            {@const tick_pos_raw = _x.format?.startsWith(`%`)
              ...
              {#if tick_pos >= pad.l && tick_pos <= width - pad.r}
                <g class="tick" transform="translate({tick_pos}, {height - pad.b})">
                  ...
                  {#if tick >= x_min && tick <= x_max}
                    {@const { x, y } = _x.tick_label_shift ?? { x: 0, y: 20 }}
                    {@const custom_label = get_tick_label(tick, _x.ticks)}
                    <text {x} {y}>
                      {custom_label ?? format_value(tick, _x.format ?? ``)}
                    </text>
                  {/if}
                </g>
              {/if}
        ...
        {#if current_x_value !== null && current_x_value !== undefined}
          {@const current_pos_raw = _x.format?.startsWith(`%`)
        ...
        {#if _x.label}
          <foreignObject
            x={width / 2 + (_x.label_shift?.x ?? 0) - 100}
            y={height - pad.b - (_x.label_shift?.y ?? -40) - 10}
            width="200"
            height="20"
          >
      ...
      <g class="y-axis">
              ...
              {#if tick_pos >= pad.t && tick_pos <= height - pad.b}
                <g class="tick" transform="translate({pad.l}, {tick_pos})">
                  ...
                  {#if tick >= y_min && tick <= y_max}
                    {@const { x, y } = _y.tick_label_shift ?? { x: -8, y: 0 }}
                    {@const custom_label = get_tick_label(tick, _y.ticks)}
                    <text {x} {y} text-anchor="end" fill={_y.color}>
                      {custom_label ?? format_value(tick, _y.format ?? ``)}
                      {#if _y.unit && idx === 0}
                        &zwnj;&ensp;{_y.unit}
                      {/if}
                    </text>
        ...
        {#if height > 0 && _y.label}
          <foreignObject
            x={-100}
            y={-10}
            width="200"
            height="20"
            transform="rotate(-90, {(_y.label_shift?.y ?? 12)}, {pad.t +
              (height - pad.t - pad.b) / 2 +
              ((_y.label_shift?.x ?? 0))}) translate({(_y.label_shift?.y ?? 12)}, {pad.t +
              (height - pad.t - pad.b) / 2 +
              ((_y.label_shift?.x ?? 0))})"
          >
            <div class="axis-label y-label" style:color={_y.color}>
              {@html _y.label ?? ``}
            </div>
          </foreignObject>
        {/if}
      ...
      {#if y2_points.length > 0}
        <g class="y2-axis">
                ...
                {#if tick_pos >= pad.t && tick_pos <= height - pad.b}
                  <g class="tick" transform="translate({width - pad.r}, {tick_pos})">
                    ...
                    {#if tick >= y2_min && tick <= y2_max}
                      {@const { x, y } = _y2.tick_label_shift ?? { x: 8, y: 0 }}
                      {@const custom_label = get_tick_label(tick, _y2.ticks)}
                      <text {x} {y} text-anchor="start" fill={_y2.color}>
                        {custom_label ?? format_value(tick, _y2.format ?? ``)}
                        {#if _y2.unit && idx === 0}
                          &zwnj;&ensp;{_y2.unit}
                        {/if}
                      </text>
          ...
          {#if height > 0 && _y2.label}
            <foreignObject
              x={-100}
              y={-10}
              width="200"
              height="20"
              transform="rotate(-90, {width - pad.r + ((_y2.label_shift?.y ?? 0))}, {pad.t +
                (height - pad.t - pad.b) / 2 +
                ((_y2.label_shift?.x ?? 0))}) translate({width -
                pad.r +
                ((_y2.label_shift?.y ?? 0))}, {pad.t +
                (height - pad.t - pad.b) / 2 +
                ((_y2.label_shift?.x ?? 0))})"
            >
              <div class="axis-label y2-label" style:color={_y2.color}>
                {@html _y2.label ?? ``}
              </div>
            </foreignObject>
          {/if}
      ...
      <!-- Zero lines -->
      {#if _display.x_zero_line && _x.scale_type === `linear` &&
        !_x.format?.startsWith(`%`) && x_min <= 0 && x_max >= 0}
        ...
        {/if}
      {/if}
      {#if _display.y_zero_line && _y.scale_type === `linear` && y_min <= 0 &&
        y_max >= 0}
        ...
        {/if}
      {/if}
      {#if _display.y_zero_line && y2_points.length > 0 &&
        _y2.scale_type === `linear` && y2_min <= 0 && y2_max >= 0}
      ...
      {#if _styles.show_lines}
        {#each filtered_series ?? [] as series_data (series_data._id)}
          ...
          <g data-series-id={series_data._id} clip-path="url(#{clip_path_id})">
            {#if series_markers?.includes(`line`)}
              {@const all_line_points = series_data.x.map((x: number, idx: number) => ({
          x,
          y: series_data.y[idx],
        }))}
              {@const finite_screen_points = all_line_points
          .map((point: { x: number; y: number }) => get_screen_coords(point, series_data))
          .filter(([sx, sy]: [number, number]) => isFinite(sx) && isFinite(sy))}
          ...
          (!has_multiple_series ||
            series_data._id === series_with_ids[selected_series_idx]?._id)}
              <Line
                ...
                ? _styles.line?.color ?? `cornflowerblue`
      ...
      {#if _styles.show_points}
        {#each filtered_series ?? [] as series_data (series_data._id)}
          ...
          : apply_controls
          ? _styles.point?.size ?? point.point_style?.radius ?? 4
          : point.point_style?.radius ?? 4}
                <ScatterPoint
                  ...
                  : apply_controls
                  ? _styles.point?.color ?? point.point_style?.fill ??
                    `cornflowerblue`
    ...
    {#if _controls.show}
      <ScatterPlotControls
        toggle_props={_controls.toggle_props}
        pane_props={_controls.pane_props}
        {x_axis}
        {y_axis}
        {y2_axis}
        {display}
