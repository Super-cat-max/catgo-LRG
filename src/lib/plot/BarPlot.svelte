<script lang="ts">
  ...
  const merged_bar = $derived({ ...DEFAULTS.bar.bar, ...bar })
  const merged_line = $derived({ ...DEFAULTS.bar.line, ...line })
  const merged_y2 = $derived({
    format: ``,
  ...
  $effect(() => { // handle x_axis.range / y_axis.range / merged_y2.range changes
    const new_x = [
    ...
    const new_y2 = [
      merged_y2.range?.[0] ?? auto_ranges.y2[0],
      merged_y2.range?.[1] ?? auto_ranges.y2[1],
    ] as [number, number]
  ...
  $effect(() => {
    const new_pad = width && height && ticks.y.length
      ? calc_auto_padding({
        ...
        y2_axis: { ...merged_y2, tick_values: ticks.y2 },
      })
    ...
    if (
      ...
      const y2_tick_width = Math.max(
        0,
        ...ticks.y2.map((tick) =>
          measure_text_width(format_value(tick, merged_y2.format), `12px sans-serif`)
        ),
      )
      // Need space for: tick shift (8px) + tick width + gap (30px) + label space (20px if present)
      const tick_shift = merged_y2.tick_label_shift?.x ?? 8
      const label_space = merged_y2.label ? 20 : 0
  ...
  let scales = $derived({
    ...
    y2: create_scale(merged_y2.scale_type ?? `linear`, ranges.current.y2, [
      height - pad.b,
      pad.t,
    ]),
  })
  ...
  let legend_data = $derived.by<LegendItem[]>(() =>
    series.map((srs: BarSeries, idx: number) => ({
      ...
      display_style: srs.render_mode === `line`
        ? {
          line_color: srs.color ?? merged_line.color,
          line_dash: srs.line_style?.line_dash,
        }
        : {
          symbol_type: `Square` as const,
          symbol_color: srs.color ?? merged_bar.color,
        },
    }))
  )
  ...
  function get_bar_data(series_idx: number, bar_idx: number, color: string) {
    ...
    const coords = { x, y, orient_x, orient_y, x_axis, y_axis, y2_axis: merged_y2 }
...
>
  {#if width && height}
    ...
    >
      ...
      {#if y2_series.length > 0 && orientation === `vertical`}
        <g class="y2-axis">
          <line
            ...
            y1={pad.t}
            y2={height - pad.b}
            stroke={merged_y2.color || `var(--border-color, gray)`}
            stroke-width="1"
          />
          {#each ticks.y2 as tick (tick)}
            {@const tick_y = scales.y2(tick as number)}
            {#if isFinite(tick_y)}
              {@const rotation = merged_y2.tick_rotation ?? 0}
              {@const shift_x = merged_y2.tick_label_shift?.x ?? 8}
              {@const shift_y = merged_y2.tick_label_shift?.y ?? 0}
              <g class="tick" transform="translate({width - pad.r}, {tick_y})">
                ...
                <text
                  ...
                  text-anchor="start"
                  dominant-baseline="central"
                  fill={merged_y2.color || `var(--text-color)`}
                  transform={rotation !== 0
                  ? `rotate(${rotation}, ${shift_x}, ${shift_y})`
                  : undefined}
                >
                  {format_value(tick, merged_y2.format)}
                </text>
              </g>
            {/if}
          {/each}
          ...
          ...ticks.y2.map((tick) =>
            ...
            {@const shift_x = merged_y2.label_shift?.x ?? 0}
            {@const shift_y = merged_y2.label_shift?.y ?? 0}
            {@const tick_shift = merged_y2.tick_label_shift?.x ?? 8}
      ...
      <g clip-path="url(#{clip_path_id})">
        ...
        {#if display.y_zero_line && y2_series.length > 0 &&
          (merged_y2.scale_type ?? `linear`) === `linear` &&
        ...
        {#each series as srs, series_idx (srs?.id ?? series_idx)}
          {#if srs?.visible ?? true}
            ...
            >
              {#if is_line}
                <!-- Render as line -->
                {@const color = srs.color ?? merged_line.color ?? `steelblue`}
                {@const stroke_width = srs.line_style?.stroke_width ?? merged_line.width ?? 2}
            ...
            : 0}
                  {@const color = srs.color ?? merged_bar.color ?? `steelblue`}
            ...
            : [Math.max(1, Math.abs(v1 - v0)), Math.max(0, Math.abs(c1 - c0))]}
                  {#if (is_vertical ? rect_h : rect_w) > 0}
                    <rect
                      ...
                      height={rect_h}
                      fill={color}
                      opacity={mode === `overlay` ? merged_bar.opacity : 1}
                      role="button"
                      tabindex="0"
    ...
    {#if hover_info && hovered}
      ...
      {@const active_y_config = hover_info.active_y_axis === `y2` ? merged_y2 : y_axis}
      <div
