<script lang="ts">
  ...
  // Set default axis labels (untrack to avoid state_referenced_locally warning;
  // the $effect below doesn't need to re-run when these props change since they're just defaults)
  untrack(() => { x_axis.label ??= `r (Å)` })
  untrack(() => { y_axis.label ??= `g(r)` })
...
<style>
  ...
  .dropped-info {
    padding: 0.5em;
    margin-bottom: 0.5em;
    background: var(--surface-bg, light-dark(#f0f0f0, rgba(255, 255, 255, 0.05)));
    border-radius: 4px;
  }
  button {
    margin-left: 1em;
