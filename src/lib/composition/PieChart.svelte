...
>
  ...
  {#if show_labels}
    {#each segments as segment (segment.element)}
      <foreignObject
        x={segment.label_x - (size * 0.2 * segment.font_scale) / 2}
        y={segment.label_y - (size * 0.12 * segment.font_scale) / 2}
        width={size * 0.2 * segment.font_scale}
        height={size * 0.12 * segment.font_scale}
        class:hovered={hovered_element === segment.element}
      >
        <div
          class="pie-label"
