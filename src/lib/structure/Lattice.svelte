...
<script lang="ts">
  ...
  let {
    ...
    vector_origin = [0, 0, 0] satisfies Vec3,
    float_fmt = `.2f`,
  }: {
  ...
  function get_cylinder_transform(
    ...
    const quaternion = new Quaternion().setFromUnitVectors(
      ...
      direction.normalize(), // safe: zero-length case returns early above
    )
    ...
    {#if show_cell_vectors}
      <T.Group position={vector_origin}>
        {#each matrix as vec, idx (vec)}
          {@const vec_length = Math.hypot(...vec)}
          {@const shaft_length = vec_length * 0.85}
          <!-- Shaft goes to 85% of vector length -->
          {@const tip_start_position = math.scale(vec, 0.85) as Vec3}
          <!-- Calculate rotation to align with vector direction (guard zero-length) -->
          {@const dir = vec_length > 1e-10 ? new Vector3(...vec).normalize() : new Vector3(0, 1, 0)}
          {@const quaternion = new Quaternion().setFromUnitVectors(
      new Vector3(0, 1, 0),
      dir,
    )}
