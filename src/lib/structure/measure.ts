...
export const MAX_SELECTED_SITES = 100 // Increased from 8 to support more atom manipulation
...
/** Dihedral angle defined by 4 points A-B-C-D.
 *  The dihedral is the angle between the plane(A,B,C) and the plane(B,C,D),
 *  measured around the B-C axis. Range: -180° to +180°. */
export function dihedral_angle(
  a: Vec3, b: Vec3, c: Vec3, d: Vec3,
  mode: AngleMode = `degrees`,
): number {
  // Vectors along the chain
  const b1: Vec3 = [b[0] - a[0], b[1] - a[1], b[2] - a[2]]
  const b2: Vec3 = [c[0] - b[0], c[1] - b[1], c[2] - b[2]]
  const b3: Vec3 = [d[0] - c[0], d[1] - c[1], d[2] - c[2]]

  // Normal vectors to planes ABC and BCD
  const n1 = cross(b1, b2)
  const n2 = cross(b2, b3)

  // Normalize b2 for the sign convention
  const b2_len = Math.hypot(b2[0], b2[1], b2[2])
  if (b2_len < 1e-12) return 0

  const m1: Vec3 = cross(n1, [b2[0] / b2_len, b2[1] / b2_len, b2[2] / b2_len])
