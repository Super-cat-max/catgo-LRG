<script lang="ts">
  ...
  let {
    ...
    coordination_computing?: boolean
    ...
    on_hidden_elements_change?: (set: Set<ElementSymbol>) => void
  ...
  let has_charges = $derived(
    structure?.sites?.some((site) => typeof site.properties?.bader_charge === `number`) ?? false,
  )

  // Show element legend for element mode, or as fallback when property colors not available
  ...
  function update_element_radius(elem: ElementSymbol, value: string) {
    ...
    console.log(`[AtomLegend] update_element_radius:`, elem, value, `parsed:`, radius)
    if (radius !== null) {
      element_radius_overrides = { ...element_radius_overrides, [elem]: radius }
      console.log(`[AtomLegend] element_radius_overrides updated:`, element_radius_overrides)
    }
  }
  ...
  function update_site_radius(site_idx: number, value: string) {
    const radius = parse_radius(value)
    console.log(`[AtomLegend] update_site_radius:`, site_idx, value, `parsed:`, radius)
    if (radius === null) return
    site_radius_overrides?.set(site_idx, radius)
    console.log(`[AtomLegend] site_radius_overrides updated, size:`, site_radius_overrides?.size)
  }
  ...
  >
    ...
    {#if mode_menu_open}
      <div class="mode-dropdown">
        {#each Object.entries(SETTINGS_CONFIG.structure.atom_color_mode.enum || {}).filter(([v]) => v !== `custom`) as
          [value, label]
          (value)
        }
          <button
            ...
            class:disabled={value === `wyckoff` && !sym_data || value === `charge` && !has_charges}
            disabled={value === `wyckoff` && !sym_data || value === `charge` && !has_charges}
            onclick={() => {
...
{#if show_element_legend}
  ...
  <div {...rest} class="atom-legend element-legend {rest.class ?? ``}" onclick={(e) => e.stopPropagation()} oncontextmenu={(e) => e.stopPropagation()}>
    {#each Object.entries(elements!) as [elem, amt], idx (elem + amt)}
      ...
      <div class="legend-item">
        ...
        <button
          ...
          onclick={(event) => {
            const new_set = toggle_visibility(hidden_elements, elem as ElementSymbol, event)
            on_hidden_elements_change?.(new_set)
          }}
    ...
    {#if coordination_computing && atom_color_config.mode === `coordination`}
  ...
  >
    {#if legend_title}
      <div class="legend-header">
        <h4>{legend_title}{#if coordination_computing && atom_color_config.mode === `coordination`} <span class="computing-indicator">(computing...)</span>{/if}</h4>
      </div>
    {/if}
