<script lang="ts">
  ...
  // Clamp head length so it never exceeds 60% of total arrow length
  const raw_head_len = $derived(
    arrow_head_length < 0 ? vec_len * -arrow_head_length : arrow_head_length,
  )
  const head_len = $derived(Math.min(raw_head_len, vec_len * 0.6))
  const shaft_len = $derived(Math.max(0.001, vec_len - head_len))
  const shaft_r = $derived(
    shaft_radius < 0 ? Math.max(0.01, shaft_len * -shaft_radius) : shaft_radius,
  )
  const head_r = $derived(
    arrow_head_radius < 0 ? Math.max(0.02, shaft_len * -arrow_head_radius) : arrow_head_radius,
  )
...
{#if shaft_len > 0.001}
  <T.Mesh {...rest} position={shaft_pos} {rotation} raycast={null}>
    <T.CylinderGeometry args={[shaft_r, shaft_r, shaft_len, 12]} />
    <T.MeshStandardMaterial {color} />
  </T.Mesh>
{/if}

<T.Mesh {...rest} position={head_pos} {rotation} raycast={null}>
