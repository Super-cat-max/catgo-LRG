...
const mock_vscode = vi.hoisted(() => ({
  ...
  RelativePattern: vi.fn(function (this: any, base: unknown, pattern: string) {
    this.base = base
...
describe(`CatGo Extension`, () => {
  ...
  beforeEach(async () => {
    ...
    ;(mock_fs.readFileSync as ReturnType<typeof vi.fn>).mockReturnValue(
      `mock content` as any,
    )
  ...
  test(`render handles errors`, async () => {
    ...
    await render(mock_context)
    expect(mock_vscode.window.showErrorMessage).toHaveBeenCalledWith(
      `Failed: No file selected. CatGo needs an active editor to know what to render.`,
    )
  })
  ...
  test(`extension activation`, async () => {
    ...
    expect(mock_vscode.window.registerCustomEditorProvider)
      .toHaveBeenCalledWith(`catgo.viewer`, expect.any(Object), expect.any(Object))
  })
  ...
  describe(`Bug Reporting`, () => {
    ...
    beforeEach(async () => {
      ...
      report_bug_command = command_registry.get(`catgo.report_bug`) ?? null
    })
    ...
    test(`should generate bug report with environment information`, async () => {
      ...
      expect(content).toContain(`- **CatGo Version**: ${pkg_json.version}`)
  ...
  describe(`Auto-Render Functionality`, () => {
    ...
    test(`should handle file reading errors gracefully during auto-render`, async () => {
      ...
      await vi.waitFor(() => {
        expect(mock_vscode.window.showErrorMessage).toHaveBeenCalledWith(
          expect.stringContaining(`CatGo auto-render failed:`),
        )
      })
    })
  })
...
H 0.0 1.0 0.0`
      ...
      const multi_frame_parsed_data = JSON.parse(
        multi_frame_html.match(/catgoData=(\{[\s\S]*?\})(?=\s*<\/script>)/)?.[1] ??
          `{}`,
      )
      ...
      const single_frame_parsed_data = JSON.parse(
        single_frame_html.match(/catgoData=(\{[\s\S]*?\})(?=\s*<\/script>)/)?.[1] ??
          `{}`,
      )
      ...
      const compressed_parsed_data = JSON.parse(
        compressed_html.match(/catgoData=(\{[\s\S]*?\})(?=\s*<\/script>)/)?.[1] ??
          `{}`,
      )
