...
impl StructureMatcher {
    ...
    fn cart_dists(
        ...
        // Get shortest vectors (ignore images, third return value)
        let (vecs, d2, _images) = pbc_shortest_vectors(avg_lattice, s2_fc, s1_fc, Some(mask), None);
    ...
    ///
    /// Returns a map from representative index to a list of all indices
    /// in that group. Structures are first bucketed by composition hash,
    /// then matched within each bucket using `fit`.
    pub fn group(&self, structures: &[Structure]) -> crate::error::Result<HashMap<usize, Vec<usize>>> {
        if structures.is_empty() {
            return Ok(HashMap::new());
        }

        // Bucket by composition hash for fast pre-filtering
        let mut buckets: HashMap<u64, Vec<usize>> = HashMap::new();
        for (i, s) in structures.iter().enumerate() {
            buckets.entry(self.composition_hash(s)).or_default().push(i);
        }

        // Within each bucket, group by structural matching
        ...
        for indices in buckets.values() {
            ...
            for i in 0..indices.len() {
                ...
                for j in (i + 1)..indices.len() {
                    ...
                    if self.fit(&structures[rep], &structures[indices[j]]) {
                        group.push(indices[j]);
                        assigned[j] = true;
                    }
                }
                groups.insert(rep, group);
            }
        }
    ...
    ///
    /// Returns a mapping where `result[i]` is the index of the representative
    /// structure that structure `i` matches. The first occurrence of each
    /// unique structure is its own representative.
    pub fn deduplicate(&self, structures: &[Structure]) -> crate::error::Result<Vec<usize>> {
        let groups = self.group(structures)?;
        let mut mapping = vec![0usize; structures.len()];
