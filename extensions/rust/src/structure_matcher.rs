...
mod tests {
    ...
    fn make_nacl_shifted() -> Structure {
        // Non-rigid perturbation: only Na is shifted, Cl stays put.
        // This changes the Na–Cl distance (genuine structural distortion),
        // unlike a global translation which the metric is invariant to.
        Structure::new(
            Lattice::cubic(5.64),
            vec![Species::neutral(Element::Na), Species::neutral(Element::Cl)],
            vec![Vector3::new(0.02, 0.0, 0.0), Vector3::new(0.5, 0.5, 0.5)],
        )
    }
