"""CP2K PDOS file reader — Tool-First format.
    ...
    "input_schema": {
        ...
        "properties": {
            "file_paths": {
                "type": "array",
                "items": {"type": "string"},
                "description": "List of .pdos file paths (one per atom kind)",
            },
            "fermi_override": {
                "type": "number",
                "description": "Override Fermi energy in eV",
            },
            "dummy_lattice": {
                "type": "number",
                "default": 10.0,
                "description": "Lattice constant for dummy cell (Angstrom)",
            },
        },
        "required": ["file_paths"],
    },
...
def detect_files(filenames: list[str]) -> bool:
    """Return True if any filename ends with .pdos."""
    return any(fn.lower().endswith(".pdos") for fn in filenames)


def priority_score(filenames: list[str]) -> int:
...
async def execute(context):
    ...
    options = {k: v for k, v in params.items() if k != "file_paths"}
